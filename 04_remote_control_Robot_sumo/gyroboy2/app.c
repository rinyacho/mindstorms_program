...
void main_task(intptr_t unused) {
	...
	ev3_motor_config( ARM, MEDIUM_MOTOR );		// 腕振りコンフィグセットアップ
    ...
    while(1) {
    	...
    	if ( z == 0 ){							// 腕振り開始（マイナスとプラスに循環）
			ev3_motor_set_power( ARM,-50);		// 腕振りマイナス
			y = y + 1;
			if ( y >= 15 ){
				z = 1;
				y = 0;
			...
			ev3_motor_set_power( ARM,50);		// 腕振りプラス
			y = y + 1;
			if ( y >= 15 ){
				z = 0;
				y = 0;
		...
		dly_tsk(10);
		ev3_motor_set_power( ARM,0);			// 腕振り終了
    	...
    	case 'a':
    		if(motor_control_steer < 0)
    			...
    			motor_control_steer += 10;		// 黄色（YELLOW）
    	...
    	case 'd':
    		if(motor_control_steer > 0)
    			...
    			motor_control_steer -= 10;		// 緑色（GREEN）
    		fprintf(bt, "motor_control_steer: %d\n", motor_control_steer);
    		break;
